...
export function App() {
    ...
    if (phase === "check_resume") {
      ...
      if (existing) {
        ...
        if (existing.avoidanceType) {
          setResult({
            type: existing.avoidanceType as AvoidanceResult["type"],
          ...
          setAgenticLog((existing.agenticLog as AgenticTurn[]) ?? []);
        }
      } else {
    ...
    (resume: boolean) => {
      if (resume && result) {
        ...
        if (sessionId) {
          ...
          trackEvent("session_abandoned", { sessionId });
        }
        setSessionId(null);
        setTask("");
  ...
  const handleTaskSubmit = useCallback((value: string) => {
    ...
    trackEvent("session_started", { sessionId: session.id });
    setPhase("prompt_spouse");
  }, []);

  const handleSpouseSubmit = useCallback(
    async (value: string) => {
      ...
      try {
        ...
        trackEvent("avoidance_classified", {
          sessionId,
  ...
  const runAgenticStep = useCallback(
    async (log: AgenticTurn[]) => {
  ...
  const handleAgenticInput = useCallback(
    (value: string) => {
  ...
  const handleTimerChoice = useCallback(
    (start: boolean) => {
      if (start) {
        if (sessionId) {
          const db = getDb();
          updateSession(db, sessionId, { timerStarted: true });
          trackEvent("timer_started", { sessionId });
        }
        setPhase("timer");
      } else {
        handleSessionEnd(false);
    ...
    case "timer_prompt":
      return (
        <box style={{ flexDirection: "column", padding: 2, gap: 1 }}>
          <text>
            <b>Start a 10 minute timer now?</b>
          </text>
          <PromptInput
            label="(y/n)"
