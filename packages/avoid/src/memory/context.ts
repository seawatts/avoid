...
import { getTopMemories } from "./decay";
import { analyzePatterns, shouldConsolidate, consolidateMemories } from "./patterns";
import { getLatestPatternSummary } from "./store";

export function buildMemoryContext(db: Database): string {
  ...
  if (topMemories.length > 0) {
    ...
    for (const memory of topMemories) {
      ...
      const prefix = memory.type === "observation" ? "Obs" : memory.type === "insight" ? "Insight" : memory.type === "pattern" ? "Pattern" : "Summary";
